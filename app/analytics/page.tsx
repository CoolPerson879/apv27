...
import { BarChart, Loader2, AlertCircle, CheckCircle, XCircle, Clock, BookOpen } from "lucide-react"
...
export default function AnalyticsPage() {
  ...
  const [practiceData, setPracticeData] = useState<UserPracticeData | null>(null)
  ...
  const lastScorePercentage =
    practiceData?.last_score !== null && practiceData?.total_questions && practiceData.total_questions > 0
      ? Math.round((practiceData.last_score / practiceData.total_questions) * 100)
      : 0
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-120px)] text-center px-4">
        ...
        <p className="text-lg text-muted-foreground mb-6">Please sign in to view your personalized analytics.</p>
        <Button asChild>
  ...
  if (loadingData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-120px)]">
        <Loader2 className="h-10 w-10 animate-spin text-primary" />
        <p className="mt-4 text-lg text-muted-foreground">Loading your analytics...</p>
      </div>
    )
  }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-lg text-muted-foreground">
          Welcome, {user.name || user.email}! Here's a summary of your practice progress.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            <div className="text-5xl font-bold text-primary">{practiceData?.questions_attempted || 0}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your performance on the most recent quiz</CardDescription>
          </CardHeader>
          <CardContent>
            {/* This check is already robust */}
            {practiceData && practiceData.last_score !== null && practiceData.total_questions !== null ? (
              <>
                <div className="text-5xl font-bold text-primary">
                  {practiceData.last_score}/{practiceData.total_questions}
                </div>
                <Progress value={lastScorePercentage} className="w-full mt-4" />
                <p className="text-sm text-muted-foreground mt-2">{lastScorePercentage}% Correct</p>
              </>
            ) : (
        ...
        <CardContent>
          <ul className="space-y-2">
            <li className="flex items-start gap-2">
              ...
              <span>
                **Review Explanations:** After each practice question, carefully read the explanation for both correct
            ...
            <li className="flex items-start gap-2">
              ...
              <span>
                **Identify Weak Areas:** Pay attention to the periods or themes where you consistently score lower.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <BookOpen className="h-5 w-5 text-blue-500 flex-shrink-0 mt-1" />
              <span>
                **Revisit Course Content:** Go back to the "Course Content" and "Course Themes" pages to reinforce
                knowledge in challenging areas.
              </span>
            </li>
            <li className="flex items-start gap-2">
